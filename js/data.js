...
let birth_reasons = ["i was born in a normal, healthy way.","i was born after my father's condom broke","i was summoned at a cult ritual","i was born in a normal way, however my mother died to due maternal death","i was an inhome birth","i fell out of the sky","i came into existence.. somehow"]
let eventPool = [
    new LifeEvent({
            ...
            presentChoice("Will you let this slide?", [
                {
                    ...
                    callback: () => {
                        if (p.strength > rand_int(50) + 40) {
                            ...
                            p.money = 0;
                            p.strength += 5; //this is Sline 67 hahahahaha // <- i dont know what the fuck she meant by that - mewity
    ...
    new LifeEvent({
        title: "Bad morning.",
        description: "You're in college.",
        chance: 1.,
        minAge: 19,
        maxAge: 19,
    }),
    new LifeEvent({
        title: "We are so barack.",
        description: "You're done with college.",
        chance: 1.,
        minAge: 21,
        maxAge: 21,
    }),
    ...
    new LifeEvent({
        ...
        effect: (p) => {
            presentChoice("Do you go?", [
                {
                    text: "Dude... Fuck, Yes. (-20$, +30 happiness).",
                    callback: () => { 
                        if (p.money >= 20) {
							if (rand_int(25)) {
								p.health -= 10;
								p.money -= 20;
								p.happiness += 10;
								print("You hurt your wrist playing maimaiDX. Fuck!")
							} else {
                            p.happiness += 30;
                            p.money -= 20;
                            print("You played some damn good games. Nerd...");
							}} else {
                            p.happiness -= 30;
                            print("You didn't have enough money. Broke boy...")
                    }
                }},
                {
                    text: "Dude... Fuck, No. (-30 happiness, +15 intelligence).",
                    callback: () => {
                        p.happiness -= 30;
                        p.intelligence += 15;
                        print("You stayed at home. Nerd...");
                    }
                }
            ]);
        }
    ...
    new LifeEvent({
	    title: "I can't see the end of the horizon...",
    	description: "Theres a Miku EXPO happening in you area."
	    chance: 0.15,
	    minAge: 14,
        maxAge: 30,
	    effect: (p) => {
		    presentChoice("Do you want to go? Tickets are really expensive...", [
			    {
				    text: "Yes. (-200$, +75 happiness)"
				    callback: () => {
					    p.money -= 200;
					    p.happiness += 75;
					    print("Worth it."); }
